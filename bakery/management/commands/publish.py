...
class Command(BasePublishCommand):
    ...
    def add_arguments(self, parser: object) -> object:
        ...
        parser.add_argument(
            ...
            default=False,
            help=(
                "Keep files in S3, even if they do not exist in the build directory."
            ),
        )
        parser.add_argument(
            "--no-pooling",
            ...
            default=False,
            help=(
                "Run uploads one by one rather than pooling them to run concurrently."
            ),
        )
    ...
    def handle(self, *args: object, **options: object) -> object:
        ...
        if not self.dry_run and not self.no_delete:
            ...
            if self.deleted_files:
                ...
                if self.verbosity > 0:
                    self.stdout.write(f"Deleting {self.deleted_files} keys")
        ...
        msg = (
            f"Publish completed, {self.uploaded_files} uploaded and "
            f"{self.deleted_files} deleted files in {elapsed_time:.2f} seconds"
        )
        logger.info(msg)
        if self.verbosity > 0:
    ...
    def set_options(self, options: object) -> object:
        ...
        # If the user does not specify a build dir...
            ...
            if not Path(settings.BUILD_DIR).exists():
                raise CommandError(self.build_missing_msg)
            # Go ahead and use it
    ...
    def get_bucket_file_list(self) -> object:
        ...
        if self.aws_bucket_prefix:
            logger.debug(
                "Adding prefix %s to bucket list as a filter", self.aws_bucket_prefix
            )
    ...
    def get_local_file_list(self) -> object:
        ...
        """
        file_list = []
        for dirpath, _dirnames, filenames in os.walk(self.build_dir):
            for fname in filenames:
                # relative path, to sync with the S3 key
                local_key = str(Path(dirpath).relative_to(self.build_dir) / fname)
    ...
    def sync_with_s3(self) -> object:
        ...
        logger.debug(
            "Comparing %s local files with %s bucket files",
    ...
    def compare_local_file(self, file_key: object) -> object:
        ...
        """
        # Where is the file?
        file_path = str(Path(self.build_dir) / file_key)
        ...
        if file_key in self.s3_obj_dict:
            ...
            # If they don't match, we want to add it
                ...
                logger.debug("%s has changed", file_key)
                self.update_list.append((file_key, file_path))
    ...
    def upload_to_s3(self, key: object, filename: object) -> object:
        ...
        if (
            self.gzip and content_type in self.gzip_content_types
        ) or encoding == "gzip":
        ...
        if content_type in self.cache_control:
            extra_args["CacheControl"] = "".join(
                ("max-age=", str(self.cache_control[content_type]))
            )
